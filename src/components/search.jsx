...
const Search = ({onSearch}) => {
  ...
  const handleKeyDown = (event) => {
    if (event.key === "Enter") {
      handleSearch(); // Call the onSearch function passed from Home.jsx
    }
  };
          ...
          {loading ? (
            ...
            <div className="movie-card_container">
              ...
              {!loading && !error && movies.length === 0 && !searchTerm.trim() && (
                <div className="one">
                  <p className="start__typing">
                    Start typing a movie title above to search!
                  </p>
                </div>
              )}
